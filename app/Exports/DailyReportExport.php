...
class DailyReportExport implements WithMultipleSheets
{
    ...
    {
        return [
            'Patient Department' => new PatientDeptSheet($this->data['patientDept']),
            'Lazer Sessions' => new LazerSheet($this->data['lazer'])
        ];
    }
}
...
class PatientDeptSheet implements FromCollection, WithTitle, WithHeadings
{
    ...
    public function collection()
    {
        return $this->data->map(function ($item) {
            ...
            return [
                'patient_name' => mb_convert_encoding($patient->name, 'UTF-8', 'auto'),
                ...
                'check_in_type' => mb_convert_encoding($item->accounter->first()->pivot->check_in_type ?? 'N/A', 'UTF-8', 'auto'),
                'given_cure' => mb_convert_encoding($item->accounter->first()->pivot->given_cure ?? 'N/A', 'UTF-8', 'auto'),
                'tools' => mb_convert_encoding($item->accounter->first()->pivot->tools ?? 'N/A', 'UTF-8', 'auto'),
